...
feature 'Browse' do
  context 'stubs displayed when no results' do
    scenario 'funde#index' do
      visit funds_path
      expect(page).to have_text("Can't find the fund you’re looking for?")
    end

    scenario 'funde#themed' do
      visit theme_path(build(:theme, slug: 'no-funds'))
      expect(page).to have_text("Can't find the fund you’re looking for?")
    end
  end
