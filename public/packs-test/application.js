...
if(__webpack_require__(9)){
  var LIBRARY             = __webpack_require__(21)
    ...
    , $typed              = __webpack_require__(47)
    , $buffer             = __webpack_require__(66)
    , ctx                 = __webpack_require__(12)
    ...
    , arrayFill           = __webpack_require__(53)
...
var $export     = __webpack_require__(0)
  , toLength    = __webpack_require__(8)
  , context     = __webpack_require__(60)
...
$export($export.S, 'Math', {sign: __webpack_require__(63)});
...
var _dialogPolyfill = __webpack_require__(189);
...
var Dialog = function () {
  ...
  _createClass(Dialog, [{
    ...
    value: function init() {
      ...
      var dialog = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'dialog';

      var $modal = document.querySelector(dialog);
      if (!$modal) return;
      _dialogPolyfill2.default.registerDialog($modal);

      document.querySelector('.js-show-modal').onclick = function () {
        $body.classList.add('js-open-modal');
        $modal.showModal();
      };

      document.querySelector('.js-close-modal').onclick = function () {
...
var __WEBPACK_AMD_DEFINE_RESULT__;(function() {
  ...
  if ("function" === 'function' && 'amd' in __webpack_require__(190)) {
    // AMD support
...
var Select = function () {
  ...
  _createClass(Select, [{
    ...
    value: function init(select, opts) {
      this._hideOpts(select, opts);
      this._selectOnChange(select, opts);
    }
  }, {
    key: 'orgType',
    value: function orgType(array) {
    ...
    key: '_hideOpts',
    value: function _hideOpts(select, opts) {
      var $select = document.getElementById(select);
      if (!$select) return;

      var selectedClasses = opts[$select.value] || [];

      var classes = [].concat.apply([], Object.values(opts)).filter(function (i) {
        return selectedClasses.indexOf(i) === -1;
      });
      ...
      classes.forEach(function (cls) {
        ...
        try {
          for (var _iterator2 = Array.from(document.getElementsByClassName(cls))[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            var el = _step2.value;
    ...
    key: '_selectOnChange',
    value: function _selectOnChange(select, opts) {
      var $select = document.getElementById(select);
      if (!$select) return;
      var self = this;
      $select.addEventListener('change', function (e) {
        ...
        if (opts[this.value]) {
          var classes = opts[this.value].map(function (i) {
            return '.' + i;
          });
          var $els = document.querySelectorAll(classes);
...
var Sort = function () {
  ...
  _createClass(Sort, [{
    ...
    key: '_filterOnChange',
    value: function _filterOnChange(id) {
      var $form = document.getElementById(id);
      if (!$form) return;

      $form.addEventListener('change', function (e) {
        e.preventDefault();
        var queryString = _queryString2.default.parse(window.location.search);
        Array.from($form.elements).forEach(function (el) {
          queryString[el.id] = el.value;
        });
        window.location.search = _queryString2.default.stringify(queryString);
      });
    }
  }]);
