module OrganisationsHelper
  ...
  def data_not_available(request)
    if @recipient.can_request_funder?(@funder, "#{request}")
      content_tag(:div, class: 'uk-alert uk-alert-warning uk-margin-bottom-remove') do
        ...
        content_tag(:p, link_to('Request', vote_recipient_path("#{request}" => true, funder_id: @funder.id, recipient_id: @recipient.id), method: 'post', class: 'uk-button uk-button-primary uk-width-1-1'))
      end
    else
  ...
  def simple_data_not_available(request)
    if @recipient.can_request_funder?(@funder, "#{request}")
      content_tag(:a, link_to('Request', vote_recipient_path("#{request}" => true, funder_id: @funder.id, recipient_id: @recipient.id), method: 'post', class: 'uk-button uk-button-primary uk-width-1-1'))
    else
      content_tag(:button, class: 'uk-button uk-width-1-1', disabled: "") do
        content_tag(:i, " Requested", class: 'uk-icon-check')
      end
