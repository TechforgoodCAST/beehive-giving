class District < ActiveRecord::Base
  ...
  # TODO: deprecated
  def rank_in_region(year = Time.zone.today.year)
    data = []
  ...
  # TODO: deprecated
  def recent_grants(year = Time.zone.today.year)
    grants.where('approved_on <= ? AND approved_on >= ?', "#{year}-12-31", "#{year}-01-01")
  end

  # TODO: deprecated
  def district_funder_ids(year = Time.zone.today.year)
    recent_grants(year).pluck(:funder_id).uniq
  end

  # TODO: deprecated
  def ids_by_grant_count(org_type, year = Time.zone.today.year)
    recent_grants(year).group("#{org_type}_id").count.sort_by { |_, v| v }.reverse.to_h
  end

  # TODO: deprecated
  def ids_by_grant_sum(org_type, year = Time.zone.today.year)
