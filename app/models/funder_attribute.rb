...
class FunderAttribute < ActiveRecord::Base
  ...
  def funded_organisation_age
    return unless funder && funder.recipients.where('approved_on < ? AND approved_on >= ?', "#{year + 1}-01-01", "#{year}-01-01").where('funding_stream = ?', funding_stream).count == funder.recipients.where('approved_on < ? AND approved_on >= ?', "#{year + 1}-01-01", "#{year}-01-01").where('funding_stream = ?', funding_stream).pluck(:founded_on).count
  ...
  def funded_organisation_income_and_staff
    return unless funder && funder.profiles.where('approved_on < ? AND approved_on >= ?', "#{year + 1}-01-01", "#{year}-01-01").count < funder.grants.where('approved_on < ? AND approved_on >= ?', "#{year + 1}-01-01", "#{year}-01-01").where('funding_stream = ?', funding_stream).count
    if funding_stream == 'All'
