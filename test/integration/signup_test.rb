...
class SignUpTest < ActionDispatch::IntegrationTest
  ...
  def fill_in_form(state, fill=true)
    within('#new_user') do
      ...
      fill_in('user_charity_number', with: '1161998') if state == 2 && fill || state == 4 && fill
      fill_in('user_company_number', with: '09544506') if state == 3 && fill || state == 4 && fill
    end
  end
  ...
  test 'filling in landing page form with correct info redirects to new organisation page' do
    ...
    assert_equal Organisation::ORG_TYPE[1][0], find("#recipient_org_type option[selected='selected']").text
  end
